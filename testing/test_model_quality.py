...
def get_best_model():
    ...
    try:
        ...
        # Logique de recherche du fichier .pkl
        client = mlflow.tracking.MlflowClient()
        artifacts = client.list_artifacts(run_id)
        
        # Cherche n'importe quel fichier finissant par .pkl à la racine
        model_filename = next((art.path for art in artifacts if art.path.endswith('.pkl')), None)
        
        # Si pas trouvé, cherche dans le sous-dossier 'model'
...
def run_quality_check():
    try:
        ...
        # IMPORTANT : open_browser=False pour ne pas bloquer Jenkins
        result.save_as_html(report_file, open_browser=False)
        
        print(f"\n✨ SUCCÈS ! Rapport généré : {full_report_path}")
        
        # Optionnel : Faire échouer le build si le résultat est mauvais
        ...
        # Indispensable pour que Jenkins marque l'étape en Rouge
        sys.exit(1)
